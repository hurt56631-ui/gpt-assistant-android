...
public class GlobalDataHolder {
    ...
    // --- 新增字段 ---
    private static String currentLanguage; // 即 ASR 语言
    ...
    // --- 兼容性修复：适配 MainActivity.java 中调用的旧/新方法名 ---

    /**
     * 获取当前 ASR 语言代码 (MainActivity.java:769)
     */
    public static String getAsrLanguage() {
        if (currentLanguage == null) return LANG_ZH;
        return currentLanguage;
    }

    /**
     * 保存 ASR 语言代码 (MainActivity.java:798)
     */
    public static void saveAsrLanguage(String lang) {
        currentLanguage = lang;
        if (sp != null) {
            sp.edit().putString("target_language", lang).apply();
        }
    }

    /**
     * 获取云端语音启用状态 (MainActivity.java:830)
     */
    public static boolean getUseCloudTts() {
        return enableCloudTts;
    }

    // --- 实例方法 (适配 TtsManager 和部分业务逻辑) ---
    ...
    public static void loadGptApiInfo() {
        ...
        String customStr = sp.getString("custom_models", "");
        if (customStr.isEmpty()) {
            customModels = new ArrayList<>();
        } else {
            customModels = new ArrayList<>(Arrays.asList(customStr.split(";")));
    ...
    public static void saveGptApiInfo(String host, String key, String model, List<String> customModelList) {
        ...
        if (sp != null) {
            ...
            for (String m : customModels) {
                if (!m.isEmpty()) sb.append(m).append(";");
            }
            editor.putString("custom_models", sb.toString());
            editor.apply();
        }
    }
